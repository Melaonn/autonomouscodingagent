...
Lifecycle transitions make the current activity visible without asking Codex to send manual progress calls. Codex proceeds from implementation directly to verification instead of running a duplicate focused test by default; focused commands remain useful when debugging a concrete problem. Verification writes durable dashboard events as each gate starts and finishes without sending progress notifications that repeatedly wake the model. It runs configured commands using argument arrays rather than model-generated shell strings. Setup can be reused only when dependency manifests, Node runtime, platform, and architecture match the recorded fingerprint. After setup, independent gates run with a concurrency limit of three. After publication, the server follows required CI on its own; status tools are reserved for recovery and explicit inspection.
...
- Codex communicates with the MCP connector through local stdio. The connector authenticates to the control-plane API with a separate device credential and uses an explicit endpoint allowlist.
- Repository and run records, events, reports, artifacts, and lifecycle mutations are authorized against the authenticated GitHub identity.
...
The MCP does not own Codex's reasoning loop, context window, compaction, or model routing. Its tool definitions and calls add model-visible input. It can avoid waste by running deterministic gates outside the model, suppressing successful logs, and waking Codex only for actionable failures, but it is not inherently a token-saving architecture. Large usage reductions require an owned harness that decides what context reaches each model call and when another model call is necessary.
...
The paired evaluator measures quality and cost instead of assuming either. Baseline and harness variants use the same task, base commit, model, reasoning effort, and hidden commands. Codex JSONL provides exact input, cached-input, output, and reasoning token counts. Experiments make no token-reduction assumption by default. An operator may explicitly configure a reduction hypothesis, which requires non-inferior quality and the target reduction in both total and uncached tokens. Insufficient trials, missing usage, unfair checkouts, and wide confidence intervals remain explicitly inconclusive. Runtime code must remain independent of benchmark repositories, prompts, task IDs, and hidden checks.
