...
The developer starts in native Plan mode. Codex calls `sdlc_start` before repository inspection. For an unknown checkout, the controller reads versioned project metadata and Git paths to detect the Git identity, default branch, TypeScript or Python stack, package manager, available scripts, test layout, source/test globs, GitHub workflows, and deployment clues. It saves a versioned draft policy and returns the evidence, warnings, and three confirmation questions. No project command runs during discovery. The dashboard shows every detected field. Coding cannot start until the developer confirms the draft in chat or reviews and saves it in the dashboard; deployment remains disabled by default.
