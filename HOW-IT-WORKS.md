...
The developer starts in native Plan mode. Codex calls `sdlc_start` before repository inspection, so the first and only discovery pass already has the applicable company rules and bounded context. The controller records the starting tree. Company documents are stored as stable chunks and ranked by PostgreSQL full-text search; the controller does not classify prompts with keyword or regular-expression rules. Codex then uses Plan mode's normal question flow. Product questions must be resolved before implementation begins.
