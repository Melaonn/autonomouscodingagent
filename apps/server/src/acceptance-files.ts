...
export function acceptanceWrites(path: string, content: string) {
  ...
  return chunks.map((chunk, index) => ({
    argv: ['node', '-e', `const fs=require('node:fs'); const path=require('node:path').join('/acceptance',process.env.SDLC_FILE); fs.${index === 0 ? 'writeFileSync' : 'appendFileSync'}(path,Buffer.from(process.env.SDLC_DATA,'base64'));${index === chunks.length - 1 ? 'fs.chmodSync(path,0o444);' : ''}`],
