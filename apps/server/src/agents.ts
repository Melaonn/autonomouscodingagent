...
export function textFrom(result: JobResult, backend: Backend) {
  ...
  for (const line of result.stdout.split(/\r?\n/)) {
    try {
      ...
      if (backend === 'codex' && event.type === 'item.completed' && event.item?.type === 'agent_message')
        text = event.item.text || text;
    } catch {
      /* streamed tools may write non-JSON diagnostics */
    }
  }
...
export function jsonFrom(text: string): unknown {
  ...
  try {
    ...
    /* find a complete trailing object */
  }
...
export class Agents {
  ...
  async structured<T>(
    ...
    prompt: string,
    schema: z.ZodType<T>,
    timeoutSeconds = run.limits.agentMinutes * 60,
  ): Promise<{ value: T; result: AgentResult }> {
    let correction = '';
    for (let attempt = 0; attempt < 2; attempt++) {
      const job = await this.runner.agent({
        runId: run.id,
        backend,
        phase,
        timeoutSeconds,
      ...
      const result: AgentResult = {
        ...
        logs: `${job.stderr}\n${job.stdout}`,
        text: textFrom(job, backend),
        usage: { inputTokens: 0, outputTokens: 0, costUsd: null },
      };
      if (job.exitCode)
        throw new Error(`${backend} ${phase} failed with exit ${job.exitCode}: ${job.stderr.slice(-1000)}`);
      try {
        const raw = jsonFrom(result.text);
  ...
  async implement(run: Run, prompt: string) {
    const result = await this.runner.agent({
      runId: run.id,
      backend: run.backend,
      phase: 'coding',
      timeoutSeconds: run.limits.agentMinutes * 60,
    ...
    if (result.exitCode)
      throw new Error(`Implementation agent failed with exit ${result.exitCode}: ${result.stderr.slice(-1000)}`);
    return result;
  }
}
