...
export class ChatClient {
  ...
  constructor(url: string, private password: string) {
  ...
  private async login() {
    ...
    const response = await fetch(`${this.url}/auth/password`, {
      ...
      headers: { 'content-type': 'application/json' }, body: JSON.stringify({ password: this.password }),
    });
    if (!response.ok) throw new Error(`Harness login failed (${response.status}). Check your local setup.`);
    this.cookie = response.headers.getSetCookie().find(value => value.startsWith('sdlc_session='))?.split(';')[0] || '';
    if (!this.cookie) throw new Error('Harness did not establish a session.');
    const me = await fetch(`${this.url}/api/me`, { headers: { cookie: this.cookie }, redirect: 'error', signal: AbortSignal.timeout(15_000) });
  ...
  async request<T>(path: string, body?: unknown): Promise<T> {
    ...
    if (!/^\/api\/(readiness|repositories|runs(?:\/[\w-]+(?:\/(answer|cancel|resume|report))?)?)$/.test(path)) throw new Error('Unsupported chat operation.');
    if (!this.cookie) {
      this.loginTask ||= this.login().finally(() => { this.loginTask = undefined; });
      await this.loginTask;
    }
    const response = await fetch(`${this.url}${path}`, {
      method: body === undefined ? 'GET' : 'POST', redirect: 'error', signal: AbortSignal.timeout(40_000),
    ...
    if (!response.ok) {
      ...
      throw new Error(`Harness request failed (${response.status}). Inspect sdlc_runs before retrying a start; use the dashboard for details.`);
    }
