...
export function validateConfig() {
  ...
  if (config.production && !config.adminPassword && !process.env.GITHUB_CLIENT_ID) throw new Error('Production requires ADMIN_PASSWORD or GitHub OAuth');
  if (config.devToken && !['localhost', '127.0.0.1', '::1'].includes(config.host)) throw new Error('Development login is loopback-only');
  if (config.adminPassword && config.adminPassword.length < 16) throw new Error('ADMIN_PASSWORD must have at least 16 characters');
