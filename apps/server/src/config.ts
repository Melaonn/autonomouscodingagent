...
export const config = {
  ...
  runnerUrl: process.env.RUNNER_URL || 'http://127.0.0.1:4311',
  runnerToken: process.env.RUNNER_TOKEN || '',
  maxActive: Number(process.env.MAX_ACTIVE_RUNS || 2),
};
export function validateConfig() {
  if (config.production && (!config.databaseUrl || !process.env.SESSION_SECRET || !config.runnerToken))
    throw new Error('Production requires DATABASE_URL, SESSION_SECRET and RUNNER_TOKEN');
