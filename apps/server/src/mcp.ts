...
export async function inspectIntegration(integration: Integration) {
  ...
  for (const [header, envName] of Object.entries(integration.headersEnv)) {
    const value = process.env[envName];
    if (!value) throw new Error(`Missing server environment variable ${envName}`);
  ...
  try {
    ...
    return listed.tools
      .filter((t) => integration.allowedTools.includes(t.name))
...
export async function callIntegration(integration: Integration, tool: string, args: Record<string, unknown>) {
  if (!integration.enabled || !integration.allowedTools.includes(tool))
    throw new Error(`MCP tool ${tool} is not approved`);
  const headers: Record<string, string> = {};
  for (const [header, envName] of Object.entries(integration.headersEnv)) {
    const value = process.env[envName];
    if (!value) throw new Error(`Missing server environment variable ${envName}`);
