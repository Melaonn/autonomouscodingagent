...
export interface Runner {
  ...
  analyze(job: { owner: string; repo: string; branch: string; prompt: string; githubToken?: string }): Promise<JobResult>;
...
export class HttpRunner implements Runner {
  ...
  prepare(runId: string, owner: string, repo: string, branch: string, githubToken?: string) { return this.request<{ sha: string }>('/prepare', { runId, owner, repo, branch, githubToken }); }
  ...
  diff(runId: string, baseSha: string) { return this.request<{ diff: string }>('/diff', { runId, baseSha }); }
  ...
  analyze(job: { owner: string; repo: string; branch: string; prompt: string; githubToken?: string }) { return this.request<JobResult>('/analyze', job); }
