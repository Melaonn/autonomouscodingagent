...
1. Inspect the current repository and `git status`, then call `sdlc_start` with the absolute repository root and the user's complete request. Use `mode: delivery` for changes. Use `mode: validation` only when the user explicitly requests a non-mutating audit, smoke test, or verification. Work in this checkout; preserve intentional local changes.
...
6. When a delivery review passes, create a `codex/` feature branch, commit the exact verified tree, push it to origin, and call `sdlc_publish`. Follow required CI with `sdlc_sync` or `sdlc_status`. A validation run completes after review only when the workspace is unchanged; report that evidence without publishing.
