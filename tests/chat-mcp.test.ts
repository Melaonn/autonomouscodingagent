  ...
  it('uses the actual API to start, recover, inspect and cancel a run without granting deployment approval', async () => {
    ...
    const repo: Repository = {
      ...repositorySchema.parse({
        ...
        repo: 'demo',
        stack: 'custom',
        checks: [{ id: 'unit', label: 'unit', argv: ['node', '--test'], kind: 'unit', report: 'exit' }],
        requiredCiChecks: ['ci'],
      }),
    ...
    expect(names).toContain('sdlc_start');
    ...
    expect(status.data.phase).toBe('planning');
    expect(status.data.gates).toEqual([]);
    ...
    record.question = 'Which priority values?';
