...
class FakeRunner implements Runner {
  ...
  async agent(job: AgentJob): Promise<JobResult> {
    let object: unknown;
    if (job.prompt.includes('delivery lead'))
      object = {
        scope: 'Add behavior',
        steps: ['implement'],
        dependencies: [],
        estimatedEffort: 'one day',
        costEstimate: 'existing subscriptions',
        schedule: ['day one'],
        risks: [],
      };
    else if (job.prompt.includes('business analyst'))
      object = {
        summary: 'Add behavior',
        criteria: [
          { id: 'AC-1', description: 'Behavior works', evidence: 'test', checkIds: ['check'], category: 'functional' },
        ],
        nonGoals: [],
        assumptions: [],
        risks: [],
    ...
    else if (job.prompt.includes('software architect'))
      object = {
        architecture: 'Follow service structure',
        apiContracts: [],
        dataChanges: [],
        uiBehavior: [],
        security: ['preserve authorization'],
        compatibility: 'compatible',
        testStrategy: 'independent test',
      };
    else if (job.prompt.includes('independent test engineer'))
      object = {
        files: [{ path: 'acceptance.test.js', content: 'process.exit(0)' }],
        command: {
          id: 'acceptance',
          label: 'acceptance',
          argv: ['node', '/acceptance/acceptance.test.js'],
          required: true,
          kind: 'acceptance',
          report: 'exit',
          reportPath: '',
    ...
    else if (job.prompt.includes('independent senior code reviewer'))
      object = {
        summary: 'Verified',
        findings: [],
        criteria: [{ id: 'AC-1', satisfied: true, evidence: 'check and acceptance gates passed' }],
      };
...
function repository(): Repository {
  ...
  return {
    ...
    branch: 'main',
    stack: 'custom',
    standards: 'Preserve authorization',
    checks: [check],
    requiredCiChecks: ['ci'],
    ciWaiver: '',
...
describe('workflow engine', () => {
  ...
  it('advances all development phases and stops at exact deployment approval', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
        ...
        if (url.endsWith('/pulls') && init?.method === 'POST')
          return Response.json({ number: 7, html_url: 'https://github.test/pr/7' });
        if (url.includes('/check-runs'))
          return Response.json({
            check_runs: [
              { name: 'ci', status: 'completed', conclusion: 'success', html_url: 'https://github.test/check' },
            ],
          });
    ...
    await store.put('repository', policy);
    const at = new Date().toISOString();
    const run: Run = {
      id: crypto.randomUUID(),
      repositoryId: policy.id,
      prompt: 'Implement the requested safe behavior',
      backend: 'codex',
      reviewer: 'codex',
      status: 'queued',
      phase: 'planning',
    ...
    const engine = new Engine(store, new FakeRunner(), 2, 0);
    for (let i = 0; i < 10; i++) await engine.runOnce();
    let current = (await store.getRun(run.id))!;
    expect(current.phase).toBe('deployment');
    expect(current.step).toBe('remote-ci');
    await engine.runOnce();
    current = (await store.getRun(run.id))!;
    expect(current.status, current.blocker).toBe('awaiting_approval');
    expect(current.prUrl).toBe('https://github.test/pr/7');
    expect(current.approval?.digest).toHaveLength(64);
    expect(current.gates.every((g) => g.status === 'pass')).toBe(true);
    await store.close();
  });
});
